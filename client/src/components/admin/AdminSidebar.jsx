...
const AdminSidebar = () => {
  ...
  return (
    <div className='h-[calc(100vh-64px)] md:flex flex-col items-center pt-8 max-w-1/3 md:max-w-60 w-full border-r border-gray-300/20 text-sm'>
                ...
                <NavLink key={index} to={link.path} end className={({isActive}) => `relative flex items-center max-md:justify-center gap-2 w-full py-2.5 md:pl-10 first:mt-6 text-gray-400 ${isActive && 'bg-primary/15 text-primary group'}`}>
                    {({isActive})=>(
                        <>
                            ...
                            <span className={`w-1 h-10 rounded-1 right-0 absolute ${isActive && 'bg-primary'}`}/>
                        </>
                    )}
